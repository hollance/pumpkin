...
public class Tween: CustomStringConvertible {
  /*
   * The object that you're tweening. This is a weak reference, so the tween will
   * automatically stop when the target is deallocated elsewhere.
   */
   ...
   */
  public func finishWithCompletion(flag: Bool) {
    if !flag {
      completionBlock = nil
    }
    finish()
  ...
  /* Called by the PPTweenPool when it's about to recycle this tween. */
...
public class BasicTween : Tween {
  ...
  /*
   * For keyframe animations; if YES, does not put the target in the start
  ...
  public override func update(dt: Float) {
    //PPAssert(self.target != nil, @"Animation should not run if target is nil");
    //PPAssert(!_completed, @"Animation should not run if it is already completed");
...
public class MoveFromTween : BasicTween {
  public var amount = GLKVector2Make(0, 0)
  private var previousValue = GLKVector2Make(0, 0)

  public override func start() {
    previousValue = PPVector2Zero
  }

  public override func step(t: Float) {
 ...
 *
 * The endColor is usually the target's normal color. You do need to specify it
 ...
 *
 * The endAlpha is usually the target's normal alpha. You do need to specify it
...
public class FadeTween : BasicTween {
  ...
  public override func step(t: Float) {
    target!.alpha = fclampf(flerpf(startAlpha, endAlpha, t), 0, 1)
  }
}
